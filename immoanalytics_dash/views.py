...
Toutes les vues Django (sans chatbot, deplace dans chatbot_groq.py).
"""
...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if not uname or not email or not pwd:
            ...
            error = "Le numero de telephone est obligatoire."
        elif pwd != pwd2:
            ...
            # Create profile with role and city
            try:
                ...
                logger.warning(f"Profile creation: {e}")

            # If seller, create optional first listing
            if role == 'seller':
                try:
                    from listings.models import Listing, ListingImage
                    l_title = request.POST.get('listing_title', '').strip()
                    l_type  = request.POST.get('listing_type', '').strip()
                    l_txn   = request.POST.get('listing_txn', 'vente').strip()
                    l_price = request.POST.get('listing_price', '').strip()
                    l_city  = request.POST.get('listing_city', '').strip()
                    l_desc  = request.POST.get('listing_desc', '').strip()

                    if l_title and l_price:
                        listing = Listing.objects.create(
                            seller=u,
                            title=l_title,
                            description=l_desc or l_title,
                            property_type=l_type or 'appartement',
                            transaction=l_txn or 'vente',
                            price=int(float(l_price)),
                            city=l_city or city or 'Dakar',
                            status='active',
                        )
                    ...
                    logger.warning(f"Listing creation at register: {e}")
...
def profile_view(request):
    ...
    user_perms = [
        ...
        {"icon": "fa-database",      "label": "Donnees completes",  "has": u.is_superuser},
        {"icon": "fa-robot",         "label": "ImmoAI Chatbot",     "has": True},
        {"icon": "fa-server",        "label": "Acces API",          "has": True},
    ]
...
def settings_view(request):
    ...
    if request.method == 'POST':
        ...
        if a == 'update_profile':
            ...
            if e: u.email = e
            u.save(); messages.success(request, "Profil mis a jour.")
            ...
            if not u.check_password(cur):
                ...
                messages.error(request, "Minimum 8 caracteres.")
            else:
...
def _demo_geo():
    ...
    return [{'t': f"{random.choice(types)} a {c[0]}", 'p': random.randint(15, 400)*1_000_000,
...
def estimation_page(request):
    ...
    price_guide = [
        ...
        {'label': 'Villa Sacre-Coeur (loyer)', 'range': '500K-3M/mois'},
...
def _parse(text):
    tl = (text.lower()
          .replace("e", "e").replace("e", "e").replace("a", "a")
...
def viewer_page(request):
    ...
    if q or city or ptype or txn or min_p or max_p or min_b:
        ...
        ai_msg = (f"{total} resultat(s) -- " + " . ".join(parts)) if parts else f"{total} resultat(s)"
...
def about_view(request):
    technologies = [
        ...
        {"name": "PostgreSQL / Neon",  "desc": "Base de donnees cloud",         "icon": "fas fa-database"},
        {"name": "XGBoost / LightGBM", "desc": "Modeles ML de prediction",      "icon": "fas fa-brain"},
        ...
        {"name": "Render / GitHub",    "desc": "Deploiement cloud",             "icon": "fas fa-cloud"},
    ]
    data_sources = [
        {"name": "CoinAfrique",   "desc": "Plateforme panafricaine",     "color": "#F59E0B", "count": "~3000 annonces"},
        {"name": "Expat-Dakar",   "desc": "Annonces expatries",          "color": "#2563EB", "count": "~2000 annonces"},
        {"name": "Loger-Dakar",   "desc": "Specialiste Dakar",           "color": "#0E6B4A", "count": "~1500 annonces"},
        {"name": "DakarVente",    "desc": "Ventes immobilieres",         "color": "#C0392B", "count": "~2500 annonces"},
        {"name": "2simmobilier",  "desc": "Nouveau scraper integre",     "color": "#0891B2", "count": "En cours"},
    ]
    stats_data = []
    try:
        from properties.models import (CoinAfriqueProperty, ExpatDakarProperty,
        ...
        total = sum(m.objects.count() for m in [
            ...
            LogerDakarProperty, DakarVenteProperty])
        stats_data = [
            {"label": "Annonces indexees",   "value": f"{total:,}"},
            {"label": "Sources de donnees",  "value": "4"},
            {"label": "Quartiers couverts",  "value": "80+"},
            {"label": "Modele ML",           "value": "XGBoost"},
        ]
    except:
        stats_data = [
            {"label": "Annonces indexees",   "value": "10K+"},
            {"label": "Sources de donnees",  "value": "4"},
            {"label": "Quartiers couverts",  "value": "80+"},
            {"label": "Modele ML",           "value": "XGBoost"},
        ]
...
def contact_view(request):
    ...
    contacts = [
        ...
        {"icon": "fas fa-phone",     "label": "Telephone", "value": "+221 XX XXX XX XX"},
        {"icon": "fas fa-map-pin",   "label": "Adresse",   "value": "Dakar, Senegal"},
